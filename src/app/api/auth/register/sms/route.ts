...
const smsRegisterSchema = z.object({
  ...
  email: z.string().email('邮箱格式不正确').optional(), // 可选
...
export async function POST(request: NextRequest) {
  try {
    ...
    const validated = smsRegisterSchema.parse(body);
    ...
    if (isDev) {
      ...
      // 生产环境验证数据库中的验证码
      codeValid = await verifySmsCode(validated.phone, validated.code, 'register');
    }
    ...
    // 检查手机号是否已存在
    const existingUser = await userManager.getUserByPhone(validated.phone);
    if (existingUser) {
      return NextResponse.json(
        { error: '该手机号已被注册' },
        { status: 400 }
    ...
    const userData = {
      ...
      email: validated.email || `user_${validated.phone}@temp.com`, // 如果没有邮箱，使用临时邮箱
